...
OPENSSL_INCLUDE := external/openssl/include/
EXPAT_INCLUDE   := external/expat/lib
SQLITE_INCLUDE  := external/sqlite/dist

LOCAL_PATH := $(PYTHON_SRC_PATH)
LOCAL_MODULE_SUBDIR := python2.6/lib-dynload/

build-module = \
   $(eval include $(CLEAR_VARS)) \
   $(eval LOCAL_MODULE := $(strip $1)) \
   $(eval LOCAL_MODULE_TAGS := optional) \
   $(eval LOCAL_C_INCLUDES := $(PYTHON_SRC_PATH) $(PYTHON_SRC_PATH)/Include $(OPENSSL_INCLUDE)) \
...
$(call build-module,  cmath ,  Modules/cmathmodule.c ,  libm )
$(call build-module,  math ,  Modules/mathmodule.c ,  libm )
$(call build-module,  strop ,  Modules/stropmodule.c )
$(call build-module,  time ,  Modules/timemodule.c ,  libm )
$(call build-module,  datetime ,  Modules/datetimemodule.c Modules/timemodule.c ,  libm )
...
LOCAL_C_INCLUDES += $(PYTHON_SRC_PATH) $(PYTHON_SRC_PATH)/Include $(REAL_LOCAL_PATH)/../../../bzip2
LOCAL_PATH := $(PYTHON_SRC_PATH)
LOCAL_MODULE := bz2
...
LOCAL_MODULE_TAGS := optional
LOCAL_PRELINK_MODULE := false
LOCAL_C_INCLUDES += $(PYTHON_SRC_PATH) $(PYTHON_SRC_PATH)/Include $(PYTHON_SRC_PATH)/Modules/zlib
LOCAL_MODULE := zlib
...
LOCAL_MODULE_TAGS := optional
LOCAL_PRELINK_MODULE := false
LOCAL_C_INCLUDES += $(PYTHON_SRC_PATH) $(PYTHON_SRC_PATH)/Include $(EXPAT_INCLUDE)
LOCAL_PATH := $(PYTHON_SRC_PATH)
...
LOCAL_PRELINK_MODULE := false
LOCAL_PATH :=  $(PYTHON_SRC_PATH)
LOCAL_C_INCLUDES += $(PYTHON_SRC_PATH) $(PYTHON_SRC_PATH)/Include $(OPENSSL_INCLUDE)
LOCAL_MODULE := _ssl
...
LOCAL_PRELINK_MODULE := false
LOCAL_PATH := $(PYTHON_SRC_PATH)
LOCAL_C_INCLUDES += $(PYTHON_SRC_PATH) $(PYTHON_SRC_PATH)/Include $(REAL_LOCAL_PATH)/../../../libffi/linux-arm
...
LOCAL_SHARED_LIBRARIES := libpython2.6 libffi libdl
include $(BUILD_SHARED_LIBRARY)
...
LOCAL_PRELINK_MODULE := false
LOCAL_PATH := $(PYTHON_SRC_PATH)
LOCAL_C_INCLUDES += $(PYTHON_SRC_PATH) $(PYTHON_SRC_PATH)/Include $(PYTHON_SRC_PATH)/Modules/_sqlite/ $(SQLITE_INCLUDE)
